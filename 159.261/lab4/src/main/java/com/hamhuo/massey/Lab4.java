...
public class Lab4 extends GameEngine {
	...
	public void updateSpaceship(double dt) {
		...
		if(Math.abs(spaceshipPositionX) > screenX || Math.abs(spaceshipPositionY) > screenY) {
			spaceshipPositionX %= screenX;
			spaceshipPositionY %= screenY;
		}

		if(spaceshipPositionY < 0 || spaceshipPositionX < 0){
			spaceshipPositionY = 500 + spaceshipPositionY;
			spaceshipPositionX = 500 + spaceshipPositionX;
		}
