...
public abstract class GameEngine implements KeyListener, MouseListener, MouseMotionListener {
    ...
    public Image loadRotatedImage(String filename, double angleDegrees) {
        try {
            ...
            transform.translate(newWidth / 2.0, newHeight / 2.0); // 移动到新画布中心
            transform.rotate(radians); // 应用旋转
            transform.translate(-source.getWidth() / 2.0, -source.getHeight() / 2.0); // 移回原图像中心
